...
export function ProductActions({ product, sourceCommunityPostId }: { product: Product; sourceCommunityPostId?: string }) {
  ...
  function runPrimaryAction() { if (purchasable) { addToCart(product, count); if (sourceCommunityPostId) { try { const current = JSON.parse(window.localStorage.getItem("pusy-community-attribution") ?? "null") as { expiresAt?: number } | null; if (!current?.expiresAt || current.expiresAt <= Date.now()) window.localStorage.setItem("pusy-community-attribution", JSON.stringify({ postId: sourceCommunityPostId, source: "community", expiresAt: Date.now() + 30 * 24 * 60 * 60 * 1000 })); } catch {} recordCommunityEvent("add_to_cart", sourceCommunityPostId, product.slug); } } else void toggleAlert("restock"); }
