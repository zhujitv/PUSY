...
export function ProductManagement({ products, categories, query, canManage, canInventoryManage, onCreate, onImport, onEdit, onAct }: Props) {
  ...
  const visibleProducts = useMemo(() => products.filter((product) => {
    const matchesQuery = !normalized || `${product.name} ${product.category} ${product.sku ?? ""} ${product.status}`.toLowerCase().includes(normalized);
  ...
  const visibleCategories = useMemo(() => categories.filter((category) => !normalized || `${category.name} ${category.parent_name ?? ""} ${category.status}`.toLowerCase().includes(normalized)), [categories, normalized]);
  ...
  return <div className="product-admin-stack">
    ...
    <section className="admin-panel product-admin-panel">
      ...
      {view === "products" ? <>
        ...
        <div className="admin-table-wrap"><table><thead><tr><th>商品</th><th>分类</th><th>价格</th><th>库存</th><th>核验状态</th><th>状态</th><th>操作</th></tr></thead><tbody>{visibleProducts.length ? visibleProducts.map((product) => <tr key={product.id}><td><div className="admin-product-cell"><Image src={product.image} alt="" width={54} height={64} sizes="54px" /><span><b>{product.name}</b><small>{product.sku ? `SKU ${product.sku}` : `商品 #${product.id}`}</small></span></div></td><td><b>{product.category}</b>{!product.category_id && <small className="category-warning">待关联分类</small>}</td><td>{formatCnyFromRub(product.price)}</td><td className={product.inventory_verified && product.stock <= product.low_stock_threshold ? "low-stock" : ""}>{product.inventory_verified ? product.stock : "—"}<small>预警 ≤ {product.low_stock_threshold}</small></td><td>{product.inventory_verified ? "已核验" : "待核验"}</td><td>{product.status === "active" ? "销售中" : "已下架"}</td><td>{(canManage || canInventoryManage) && <button className="admin-text-button" onClick={() => onEdit(product)}>{canManage ? "编辑" : "更新库存"}</button>}{canManage && product.status === "active" && <button className="admin-text-button danger" onClick={() => void onAct({ action: "archive-product", id: product.id })}>下架</button>}</td></tr>) : <tr><td className="admin-empty" colSpan={7}>当前筛选条件下没有商品</td></tr>}</tbody></table></div>
      </> : <>
        <div className="category-admin-intro"><div><b>分类结构</b><p>支持一级、二级分类、前台导航、排序和启停；分类链接由系统自动生成并保持稳定。</p></div><span>{visibleCategories.length} 个分类</span></div>
        <div className="admin-table-wrap"><table><thead><tr><th>分类名称</th><th>上级分类</th><th>商品数</th><th>排序</th><th>状态</th><th>操作</th></tr></thead><tbody>{visibleCategories.length ? visibleCategories.map((category) => <tr key={category.id}><td><b>{category.parent_id ? `↳ ${category.name}` : category.name}</b><small>{category.description || "暂无分类说明"}</small></td><td>{category.parent_name || "一级分类"}</td><td><b>{category.product_count}</b></td><td>{category.sort_order}</td><td><span className={`category-status ${category.status}`}>{category.status === "active" ? "已启用" : "已停用"}</span></td><td>{canManage ? <><button className="admin-text-button" onClick={() => setCategoryEditor(category)}>编辑</button><button className="admin-text-button danger" disabled={category.product_count > 0} title={category.product_count > 0 ? "有关联商品的分类不能删除" : undefined} onClick={() => void deleteCategory(category)}>删除</button></> : "—"}</td></tr>) : <tr><td className="admin-empty" colSpan={6}>还没有符合条件的分类</td></tr>}</tbody></table></div>
      </>}
    </section>
