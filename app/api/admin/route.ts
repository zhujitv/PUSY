...
export async function GET() {
  try {
    ...
    const [products, orders, orderItems, members, subscribers, returns, retailPartnerships, coupons, giftCards, stats, revenueTrend, providers, payments, refunds, paymentEvents, notificationSettings, notificationTemplates, notificationJobs, reviews, content, supportThreads, supportMessages, returnEvents, invoices, cannedReplies, orderStatusAnalytics, topProducts, customerAnalytics, returnAnalytics] = await Promise.all([
      ...
      db.prepare("SELECT COUNT(*) AS order_count, COALESCE(SUM(CASE WHEN o.status NOT IN ('待付款','支付失败','已取消','已退款') THEN GREATEST(o.total - COALESCE((SELECT ROUND(SUM(r.amount_fen) / 12.0)::INTEGER FROM refunds r WHERE r.order_id = o.id AND r.status = 'succeeded'), 0), 0) ELSE 0 END), 0) AS revenue, COUNT(*) FILTER (WHERE o.status IN ('待处理','已确认','配货中','已发货','退款中')) AS pending_count, COALESCE(AVG(CASE WHEN o.status NOT IN ('待付款','支付失败','已取消','已退款') THEN GREATEST(o.total - COALESCE((SELECT ROUND(SUM(r.amount_fen) / 12.0)::INTEGER FROM refunds r WHERE r.order_id = o.id AND r.status = 'succeeded'), 0) END), 0) AS avg_order_value, (SELECT COUNT(*) FROM products WHERE status = 'active' AND inventory_verified = 1 AND stock < 10) AS low_stock_count, (SELECT COUNT(*) FROM products WHERE status = 'active' AND inventory_verified = 0) AS unverified_inventory_count, (SELECT COUNT(*) FROM subscribers WHERE status = 'active') AS active_subscribers, (SELECT COUNT(*) FROM returns WHERE status = '待审核') AS pending_returns, (SELECT COUNT(*) FROM retail_partnerships WHERE status = '待联系') AS pending_partnerships FROM orders o").first(),
      db.prepare("SELECT substr(o.created_at, 1, 10) AS day, COALESCE(SUM(GREATEST(o.total - COALESCE((SELECT ROUND(SUM(r.amount_fen) / 12.0)::INTEGER FROM refunds r WHERE r.order_id = o.id AND r.status = 'succeeded'), 0), 0)), 0) AS revenue, COUNT(*) AS orders FROM orders o WHERE o.created_at::timestamp >= CURRENT_TIMESTAMP - INTERVAL '29 days' AND o.status NOT IN ('待付款','支付失败','已取消','已退款') GROUP BY substr(o.created_at, 1, 10) ORDER BY day").all(),
