...
export function NotificationsClient({ initialNotifications }: { initialNotifications: CommunityNotification[] }) {
  ...
  return <section className="community-notifications-panel">
    ...
    {notifications.length ? <div className="community-notification-list">{notifications.map((item) => {
      ...
      return <article className={item.read_at ? "read" : "unread"} key={item.id}><i>{item.event_type === "new_follower" ? <CommunityIcon name="user" /> : item.event_type === "following_post" ? <CommunityIcon name="spark" /> : <CommunityIcon name="shield" />}</i><div><span>{item.read_at ? "已读" : "新通知"}</span><h2><a href={href}>{copy.title}</a></h2><p>{copy.detail}</p><time dateTime={item.created_at}>{new Date(item.created_at).toLocaleString("zh-CN", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</time></div>{!item.read_at && <button disabled={busy} onClick={() => void markRead(item.id)} aria-label={`将“${copy.title}”标为已读`}>标为已读</button>}</article>;
