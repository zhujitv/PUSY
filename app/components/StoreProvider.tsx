...
function readStored<T>(key: string, fallback: T): T {
  ...
  try {
    ...
    const parsed = JSON.parse(value) as T | { version?: number; data?: T };
    if (parsed && typeof parsed === "object" && "version" in parsed && "data" in parsed) return parsed.version === STORAGE_VERSION && parsed.data !== undefined ? parsed.data : fallback;
    return parsed as T;
  } catch {
    return fallback;
  }
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { if (ready) window.localStorage.setItem(CART_KEY, JSON.stringify({ version: STORAGE_VERSION, data: cart })); }, [cart, ready]);
  useEffect(() => { if (ready) window.localStorage.setItem(WISHLIST_KEY, JSON.stringify({ version: STORAGE_VERSION, data: wishlist })); }, [wishlist, ready]);
  useEffect(() => {
