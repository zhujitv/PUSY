...
import { PurchaseExperienceTemplate } from "../../../publish/PurchaseExperienceTemplate";

export function CreatorPostEditor({ post, topics, products }: { post: { id: string; title: string; body: string; updatedAt: string; mediaIds: string[]; topicSlugs: string[]; productSlugs: string[]; experience: CommunityExperience }; topics: CommunityTopic[]; products: CommunityProductOption[] }) {
  ...
  async function save(intent: "draft" | "submit") {
    ...
    try {
      const response = await fetch(`/api/community/posts/${post.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ title, body, topicSlugs: topicSlug ? [topicSlug] : [], productSlugs, experienceSkinType: experience.skinType, experienceUsagePeriod: experience.usagePeriod, experienceScene: experience.scene, experienceRating: experience.rating, experienceHighlights: experience.highlights, experienceCautions: experience.cautions, expectedUpdatedAt: post.updatedAt, intent }) });
  ...
  return <div className="creator-editor-layout"><section className="creator-editor-form"><header><span>EDIT POST</span><h1>继续完善这篇分享</h1><p>公开内容修改后会重新进入审核；历史版本仅用于审计，不会公开展示。</p></header>
    ...
    <PurchaseExperienceTemplate tasks={[]} selectedTaskId={null} experience={experience} onTask={() => {}} onExperience={setExperience} />
