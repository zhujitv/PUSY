...
export default function CartPage() {
  ...
  return <PageShell><main className="commerce-page"><header><p>首页 / 购物袋</p><h1>购物袋</h1></header>{cart.length === 0 ? <section className="large-empty"><h2>购物袋还是空的</h2><p>去看看 PÚSY 的新品与畅销单品。</p><a className="primary-link" href="/catalog/products">浏览全部商品</a></section> : <>
    ...
    {recommendations.length > 0 && <section className="cart-recommendations"><div className="section-heading"><h2>顺手带上</h2><a href="/catalog/products">查看更多</a></div><div>{recommendations.map((product) => <article key={product.slug}><a href={productDetailHref(product)}><Image src={product.image} alt={product.name} width={700} height={727} sizes="(max-width: 700px) 50vw, 25vw" /><h3>{product.name}</h3></a><p>{formatCnyFromRub(product.price)}</p><button onClick={() => addToCart(product)}>加入购物袋</button></article>)}</div></section>}
  </>}</main></PageShell>;
}
