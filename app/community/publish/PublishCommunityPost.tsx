...
export function PublishCommunityPost({ displayName, topics, products, campaigns, purchaseTasks, defaultTopic, defaultProduct, defaultCampaign }: { displayName: string; topics: CommunityTopic[]; products: CommunityProductOption[]; campaigns: CommunityCampaign[]; purchaseTasks: CommunityPurchaseShareTask[]; defaultTopic?: string; defaultProduct?: string; defaultCampaign?: string }) {
  ...
  async function submit(form: HTMLFormElement, intent: "draft" | "submit" = "submit") {
    setError("");
    if (intent === "submit" && !images.length) { setError("请至少上传 1 张图片"); return; }
    if (intent === "draft" && !title.trim() && !body.trim() && !images.length) { setError("草稿至少需要标题、正文或图片中的一项"); return; }
  ...
  if (published) return <section className="community-publish-success"><i>✓</i><p>{savedAsDraft ? "DRAFT SAVED" : "SUBMITTED FOR REVIEW"}</p><h1>{savedAsDraft ? "草稿已保存" : "分享已提交"}</h1><span>{message}。</span><div><a href={memberUrl}>{savedAsDraft ? "进入创作中心" : "查看我的社区主页"}</a><a href="/community">返回社区首页</a></div></section>;

  return <div className="community-publish-layout">
    <form className="community-publish-form" onSubmit={(event) => { event.preventDefault(); void submit(event.currentTarget, "submit"); }}>
      ...
      <div className="community-publish-actions"><button type="button" className="community-draft-button" disabled={busy || (!title.trim() && !body.trim() && !images.length)} onClick={(event) => { const form = event.currentTarget.closest("form"); if (form) void submit(form, "draft"); }}>{busy ? "正在处理…" : "保存草稿"}</button><button className="community-submit" disabled={busy || body.length < 10 || !images.length}>{busy ? "正在处理…" : "提交社区审核"}</button></div>
    </form>
