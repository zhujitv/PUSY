...
export function ContentAdmin({ content, onAct }: { content: SiteContent; onAct: (payload: Record<string, unknown>) => Promise<boolean> }) {
  return <section className="admin-panel"><div className="admin-panel-title"><div><h2>首页内容运营</h2><p>保存后首页会自动读取最新文案，无需修改代码。</p></div><a href="/" target="_blank" rel="noopener noreferrer">预览首页</a></div><form className="content-admin-form" onSubmit={(event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget).entries()); void onAct({ action: "update-site-content", content: values }); }}>{contentFields.map(([key, label, help]) => <label key={key}>{label}{key === "hero_title" ? <textarea name={key} defaultValue={content[key] ?? ""} rows={3} /> : <input name={key} defaultValue={content[key] ?? ""} />}<small>{help}</small></label>)}<button className="admin-save">保存首页内容</button></form></section>;
}
