...
export async function ensureMember(identity: AccountIdentity) {
  ...
  await db.prepare("UPDATE members SET total_orders = (SELECT COUNT(*) FROM orders WHERE member_id = ? AND status != '已取消'), total_spent = COALESCE((SELECT SUM(total) FROM orders WHERE member_id = ? AND status != '已取消'), 0), updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(member.id, member.id, member.id).run();
