  ...
  return <>
    ...
    {visible.length ? <div className="catalog-grid">
      {visible.map((product) => <article className="catalog-card" key={product.slug}>
        <a href={`/products/${product.slug}`} className="catalog-image">{product.badge && <span className="badge">{product.badge}</span>}<img src={product.image} alt={product.name} /></a><button className={`wishlist-button ${isWishlisted(product.slug) ? "active" : ""}`} onClick={() => toggleWishlist(product.slug)} aria-label={isWishlisted(product.slug) ? "取消收藏" : "收藏商品"}>♡</button>
