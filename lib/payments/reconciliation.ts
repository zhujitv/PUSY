...
export type ReconciliationRow = {
  ...
  payment_status: string; order_status: string; amount_fen: number; succeeded_refund_fen: number; pending_refund_fen: number;
...
export function reconciliationAnomalies(row: Omit<ReconciliationRow, "anomalies">) {
  ...
  if (["paid", "partially_refunded", "refunding", "refunded"].includes(row.payment_status) && !row.provider_transaction_id) anomalies.push("缺少渠道交易号");
...
export async function paymentReconciliation(limit = 1000) {
  ...
  const rows = await db.prepare(`SELECT p.id AS payment_id, p.order_id, p.provider, p.merchant_trade_no, p.provider_transaction_id,
    p.status AS payment_status, o.status AS order_status, p.amount_fen,
