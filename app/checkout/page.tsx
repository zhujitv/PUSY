...
export default function CheckoutPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const order = { id: `PUSY${Date.now().toString().slice(-8)}`, createdAt: new Date().toISOString(), total: finalTotal, items: cart, customer: String(form.get("name") ?? ""), delivery };
    ...
    const savedOrder = { ...order, total: Number(result.total ?? order.total) };
    ...
    const paymentResponse = await fetch("/api/payments", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ orderId: order.id, provider, token: paymentToken }) });
    ...
    window.location.href = `/checkout/payment?orderId=${encodeURIComponent(order.id)}&provider=${provider}&token=${encodeURIComponent(paymentToken)}`;
  }
