...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (slug === "gift-card") return { title: "PÚSY 礼品卡｜PÚSY 中国官方网站", alternates: { canonical: "/gift-card" } };
...
export default async function CatalogPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  if (slug === "gift-card") redirect("/gift-card");
  const categories = await loadCategories();
  ...
  const categoryLinks = [{ slug: "products", name: "全部商品", href: "/catalog/products" }, ...categories.map((category) => ({ slug: category.slug, name: category.parent_name ? `${category.parent_name} / ${category.name}` : category.name, href: category.slug === "gift-card" ? "/gift-card" : `/catalog/${category.slug}` }))];
  return <PageShell><main className="catalog-page"><div className="catalog-title"><p>首页 / 商品目录</p><h1>{title}</h1></div><div className="category-pills">{categoryLinks.map((category) => <a className={category.slug === slug ? "active" : ""} href={category.href} key={category.slug}>{category.name}</a>)}</div><CatalogClient initialProducts={selected} categoryFilter={categoryFilter} /></main></PageShell>;
}
