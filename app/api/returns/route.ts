...
export async function POST(request: Request) {
  try {
    ...
    const order = await db.prepare("SELECT id, email, member_id, customer FROM orders WHERE upper(id) = ?").bind(orderId).first<{ id: string; email: string; member_id: number | null; customer: string }>();
