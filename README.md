...
社区第一期复用会员会话与独立后台审核权限；会员图文默认进入待审核，只有通过审核的内容与媒体可公开访问。第二期已接入会员关注、社区话题、关注内容流和站内通知。第三期接入点赞、评论与回复、收藏、用户举报和互动统计；评论删除采用状态变更，举报处理进入独立事件与后台审计日志。第四期接入商品关联、真实已购标识、社区搜索与排序、商品页会员分享、后台精选运营及曝光—点击—加购统计；统计事件以随机事件键幂等写入，不在社区事件表保存 IP。
