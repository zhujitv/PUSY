...
export function ReturnForm() {
  ...
  return <form className="return-form" onSubmit={submit}>
    ...
    <label>售后诉求<select name="requestType" required defaultValue="refund"><option value="refund">退货退款</option><option value="exchange">换货</option><option value="reship">漏发 / 错发补寄</option></select></label>
    ...
    <div className="return-form-grid"><label>退回物流公司（选填）<input name="returnCarrier" maxLength={60} placeholder="审核通过并寄回后填写" /></label><label>退回物流单号（选填）<input name="returnTrackingNumber" maxLength={64} placeholder="请输入快递单号" /></label></div>
