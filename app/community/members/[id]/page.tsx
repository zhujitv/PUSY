...
import { getCommunityMember, listCommunityPosts } from "../../../../lib/community/posts";
...
export default async function CommunityMemberPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PageShell><main className="community-member-page">
    <section className="community-member-hero">
      ...
      <aside><span>公开分享<b>{member.post_count}</b></span><span>社区阶段<b>01</b></span>{isOwner ? <a href="/community/publish">发布新分享</a> : <em>关注功能第二期开放</em>}</aside>
    </section>
    ...
    {posts.length ? <section className="community-feed member-feed">{posts.map((post) => <CommunityPostCard post={post} showStatus={isOwner} key={post.id} />)}</section> : <section className="community-empty"><span>NO NOTES YET</span><h2>{isOwner ? "还没有发布分享" : "还没有公开分享"}</h2><p>{isOwner ? "上传第一篇图文，让真实灵感被更多人看见。" : "审核通过的内容会展示在这里。"}</p>{isOwner && <a href="/community/publish">发布第一篇 →</a>}</section>}
  </main></PageShell>;
}
