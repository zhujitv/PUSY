...
export async function GET(request: Request) {
  try {
    ...
    const [reviews, summary] = await Promise.all([
      db.prepare("SELECT id, reviewer_name, rating, title, body, verified_purchase, created_at FROM product_reviews WHERE product_slug = ? AND status = 'approved' ORDER BY created_at DESC LIMIT 100").bind(slug).all(),
